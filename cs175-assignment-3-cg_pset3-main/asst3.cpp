...
static void motion(GLFWwindow *window, double x, double y) {
    ...
    if (g_mouseLClickButton && !g_mouseRClickButton && !g_spaceDown) { // left button down?
        ...
        m = Matrix4::makeTranslation(Cvec3(dx_translate, dy_translate, 0) * 0.01);
        ...
        m = Matrix4::makeTranslation(Cvec3(0, 0, -dy_translate) * 0.01);
    }
    ...
    if (g_curManipulatingObjIdx == 0) {
        ...
        if (g_mouseLClickButton && !g_mouseRClickButton && !g_spaceDown) {
            if (g_curSkyFrameIdx == 0) {
                // Orbit
                Matrix4 world_skyFrame = linFact(g_skyRbt);
                g_skyRbt = world_skyFrame * Matrix4::makeXRotation(-dy_rotate) * inv(world_skyFrame) * g_skyRbt;
                g_skyRbt = Matrix4::makeYRotation(dx_rotate) * g_skyRbt;
            } else {
                // Ego Motion
                Matrix4 sky_worldFrame = transFact(g_skyRbt);
                Matrix4 sky_skyFrame = g_skyRbt;
                g_skyRbt = sky_skyFrame * Matrix4::makeXRotation(-dy_rotate) * inv(sky_skyFrame) * g_skyRbt;
                g_skyRbt = sky_worldFrame * Matrix4::makeYRotation(dx_rotate) * inv(sky_worldFrame) * g_skyRbt;
            }
        } else {
